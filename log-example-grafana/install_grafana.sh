...
docker run -d --name logstash -p 4560:4560 \
  -v $LOGSTASH_CONF:/usr/share/logstash/config/logstash.yml -v $LOGSTASH_PIPELINE_CONF:/usr/share/logstash/pipeline/logstash.conf \
...
sudo chown 1000:1000 $GRAFANA_CONF
...
docker run -d --name grafana -p 3000:3000 -v "$GRAFANA_CONF:/etc/grafana/grafana.ini" grafana/grafana:6.1.4
